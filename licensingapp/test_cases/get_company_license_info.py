...
class GetCompanyLicenseInfoTests(APITestCase):
    ...
    def test_success_with_active_license(self):
        print("get_company_license_info test_success_with_active_license Test successful retrieval with an active license")
        self.client.credentials(HTTP_AUTHORIZATION='Bearer ' + self.admin_access_token)
        response = self.client.get(self.get_url)
        print(f"get_company_license_info test_success_with_active_license Response Status Code: {response.status_code}")
        print(f"get_company_license_info test_success_with_active_license Response Data: {response.data}")
    ...
    def test_success_no_active_license(self):
        print("get_company_license_info test_success_no_active_license Test successful retrieval when no active license")
    ...
    def test_unauthenticated(self):
        print("get_company_license_info test_unauthenticated Executing test: unauthenticated access")
        self.client.credentials()
        response = self.client.get(self.get_url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_not_admin_role(self):
        print("get_company_license_info test_not_admin_role Executing test: not admin role")
    ...
    def test_admin_employee_not_found(self):
        print("get_company_license_info test_admin_employee_not_found Executing test: admin employee not found")
